...
const WORKSPACE_SLUG = "acme-capital";

async function selectWorkspace(page) {
  await page.goto("/");
  await page.getByTestId(`workspace-card-${WORKSPACE_SLUG}`).click();
  await expect(page).toHaveURL(new RegExp(`/${WORKSPACE_SLUG}/dashboard`));
}
