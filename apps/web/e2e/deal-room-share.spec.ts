 ...
 * - 高级功能：Visitor Ask（Ask Host）、文件请求、索引文件
 * - 激活 / 禁用开关与二次确认
 * - 未保存更改关闭确认
 * - 表单校验
...
test("creates a highly constrained link and verifies every public gate", async ({ page }) => {
  ...
  // Advanced features — Visitor Ask (Ask Host only).
  ...
  await expect(allowedPage.getByText(/Ask|Visitor Ask|Ask Host/i).first()).toBeVisible({ timeout: 5000 }).catch(() => {});
