-- Split "session reused but link security config changed / gates unsatisfied"
-- out of security_gate_failed so successful re-authentication is not labeled
-- as a gate failure in access audit.
...
BEGIN
  FOR r IN
    ...
    WHERE c.contype = 'c'
      ...
      AND pg_get_constraintdef(c.oid) NOT LIKE '%session_security_config_changed%'
  LOOP
...
-- Backfill historical rows that were mislabeled as gate failures.
UPDATE security_events
SET event_type = 'session_security_config_changed'
WHERE event_type = 'security_gate_failed'
  AND reason = 'session_security_config_changed';
