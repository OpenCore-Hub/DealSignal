-- Soft screenshot-protection telemetry: visitor best-effort capture_attempt events.
-- Single attempts stay evidence-only; Compiler/rules escalate with thresholds.
...
BEGIN
  FOR r IN
    ...
    WHERE c.contype = 'c'
      ...
      AND pg_get_constraintdef(c.oid) NOT LIKE '%capture_attempt%'
  LOOP
...
CREATE INDEX IF NOT EXISTS idx_security_events_link_capture_attempt_24h
    ON security_events (link_id, created_at DESC)
    WHERE event_type = 'capture_attempt';
