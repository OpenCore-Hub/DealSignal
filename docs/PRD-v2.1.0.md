...
- docs/TRACKING-REQUIREMENTS-v2.1.0.md
- docs/UI-DESIGN-DELIVERABLE-v2.1.0.md
acceptance_criteria: AC-01 ~ AC-32
open_questions: []
ai_red_flags:
- 不得在缺少验收标准（AC）的情况下新增功能
- 不得修改已批准的 v2.1.0 范围；变更必须走变更请求
- 所有 P0 需求必须对应可验证的验收标准与埋点
...
# PRD：DealSignal — 智能文档分享与交易信号平台 v2.1.0
...
> **编写日期**：`2024-01-15`  
> **最后更新**：`2026-06-20`  
> **评审人**：产品负责人、技术负责人、设计负责人、测试负责人、运营负责人、合规负责人  
> **关联架构图**：`docs/ARCHITECTURE-v2.1.0.md`  
> **AI 置信度**：`high`  
> **AI 红线**：执行前必须通读本 PRD 的 `ai_red_flags` 与第 6 节「产品原则与设计约束」。

---

## 0. 资源使用说明

本文档为 DealSignal v2.1.0 的产品需求文档（PRD），基于 `docs/templates/PRD-template-v2.md` 模板编制，面向产品、设计、开发、测试、运营、合规六方协作。所有 `FR`、`AC`、`EVT`、`API`、`TASK`、`R`、`D` 编号在本 PRD 内唯一且可追溯。

> **版本基线**：`docs/PRD-v2.0.7-ARCHIVED.md` 已被冻结归档，自本文档发布之日起不再作为开发实施依据。
...
| 顺序 | 内容 | 说明 |
|------|------|------|
| 1 | YAML front matter | 获取版本、状态、owner、关联文档、`open_questions`、`ai_red_flags`、`pending_confirmation` |
| 2 | 第 0 节使用说明 | 理解模板目标与裁剪策略 |
| 3 | 第 2 节执行摘要 / 第 4 节范围边界 | 理解 why / what / who / 不做 |
| 4 | 第 8 节功能需求（仅 P0/P1） | 优先加载高优先级需求与验收标准 |
| 5 | 第 9 节非功能需求 | 加载性能、安全、合规约束 |
| 6 | 其他章节按需加载 | 上下文受限时可跳过详细用户故事、埋点、GTM 等 |

**分块策略**：
- 若全文超过模型上下文上限 60%，先读取摘要、范围、边界、P0 需求与验收标准。
- 实现细节（UI 文案、完整埋点列表）在首轮可跳过，按需拉取。
- `ai_red_flags` 与 `pending_confirmation` 必须在任何生成/修改动作前加载。

### 0.2 基线说明

- 本 PRD 整合了已冻结的 `docs/PRD + 产品设计的完整文档草案.md` 与 `docs/上传-查看-AI问答设计文.md` 中的核心结论。
- 后续需求变更必须更新变更日志，并重新评估依赖、验收标准与风险。
...
| 评审轮次 | 日期 | 参与人 | 结论 | 待办 |
|----------|------|--------|------|------|
| 产品内审 | 2026-06-20 | 产品团队 | 通过 | 无 |
| 技术评审 | 2026-06-20 | 技术负责人 | 通过 | 接受当前假设 |
| 设计评审 | 2026-06-20 | 设计负责人 | 通过 | 接受当前假设 |
| 测试评审 | 2026-06-20 | 测试负责人 | 通过 | 接受当前假设 |
| 合规评审 | 2026-06-20 | 合规负责人 | 通过 | 接受当前假设 |
| 最终批准 | 2026-06-20 | 产品负责人 / 项目发起人 | 已批准 | PRD-v2.1.0 进入 TDD 阶段 |

### 1.3 关联文档

| 文档类型 | 名称 | 路径/链接 | 说明 |
|----------|------|-----------|------|
| 当前 PRD | 《PRD-v2.1.0》 | `docs/PRD-v2.1.0.md` | 本文档，已批准 |
...
**DealSignal** 是一个面向融资创始人、投资机构、B2B 销售团队的智能文档分享与交易信号平台。本次 v2.1.0 交付在 MVP 基础上重点补齐 **文档上传解析、安全查看、AI 悬浮助手问答与自动定位** 三大核心能力，形成"上传 → 解析 → 分享 → 阅读 → 问答 → 洞察 → 跟进"的完整闭环。
...
阶段 1：认知
├── 触点：同行推荐 / 搜索结果 / 社交媒体
├── 用户行为：了解 DealSignal 能追踪文档阅读并识别投资人/客户兴趣
├── 痛点：不确定是否值得替换邮件附件或 DocSend
└── 机会：清晰的价值主张 + 案例（融资创始人识别高意向投资人）

阶段 2：激活
├── 触点：注册 / 创建 Workspace / 上传第一份文档
├── 用户行为：完成上传 → 创建链接 → 发送给第一位接收方
├── 痛点：担心敏感材料泄露、权限设置复杂
└── 机会：低摩擦权限默认 + 实时上传/解析状态反馈

阶段 3：使用
├── 触点：Dashboard / Viewer / AI 助手 / 数据分析页
├── 用户行为：查看谁看了什么页、使用 AI 问答、管理数据室
├── 痛点：AI 回答不准、热度评分不可解释
└── 机会：evidence 引用 + 评分因子透明 + 跟进建议可执行

阶段 4：留存
├── 触点：高意图邮件提醒 / Slack 通知 / 每周洞察简报
├── 用户行为：持续发送材料、团队协作、升级到付费计划
├── 痛点：价值感知下降、团队权限管理繁琐
└── 机会：自动化跟进建议 + 数据室模板 + 多 Workspace 管理

阶段 5：推荐
├── 触点：接收方在 Viewer 页看到品牌页与“我也要用”入口
├── 用户行为：接收方注册成为新 Workspace 的发件人
├── 痛点：接收方体验差会反向损害品牌
└── 机会：专业、顺滑、可信的 Viewer 与 AI 体验
```
...
> 完整系统架构图与部署拓扑图已迁移至 `docs/ARCHITECTURE-v2.1.0.md` 第 2、3 章。
>
> 参见：[ARCHITECTURE-v2.1.0.md#2-系统架构图](../ARCHITECTURE-v2.1.0.md#2-系统架构图)
...
### 9.7 可访问性（Accessibility）

| 场景 | 要求 | 验收标准 |
|------|------|----------|
| 键盘导航 | Viewer、AI 对话框、Dashboard 核心路径支持 Tab / Enter / Esc | 通过键盘可完成打开链接、翻页、展开 AI、提交问题 |
| 屏幕阅读器 | 关键元素具备语义化标签与 ARIA 角色 | Lighthouse Accessibility 评分 ≥ 90 |
| 色彩对比 | 正文与背景对比度 ≥ 4.5:1 | 通过 axe / WAVE 扫描 |
| 焦点可见 | 所有可交互元素焦点状态可见 | 手工走查通过 |

### 9.8 数据保留与合规

| 数据类型 | 默认保留周期 | 可配置周期 | 说明 |
|----------|--------------|------------|------|
| 原始文档与渲染文件 | 永久 | 1 年 / 90 天 / 永久 | 租户可在设置中调整 |
| 阅读行为事件 | 1 年 | 90 天 / 1 年 / 永久 | 用于热度评分与审计 |
| AI 问答日志 | 1 年 | 90 天 / 1 年 / 永久 | 用于质量分析与审计 |
| 审计日志 | 永久 | 不可缩短 | 满足合规与追溯要求 |
...
> 详见 `docs/database-model-v2.1.0.md`。本章仅列出与本次 PRD 强相关的核心表。
>
...
> **API 基础路径**
> - 内部 Workspace 接口：`https://{tenantSlug}.dealsignal.com/{workspaceSlug}/api/v1`
> - 公开访问接口：`https://{publicDomain}/api/v1/public`（API-09 亦可使用自定义域名根路径 `/?tenant=...&workspace=...&token=...`）
>
> **Workspace 上下文传递**：内部接口通过子域名确定 tenant，通过 URL 路径 `/{workspaceSlug}/...` 确定 workspace。API-09 为公开链接落地页，使用品牌方自定义域名，通过 query 参数传递 tenant/workspace/token。API-14 为公开访问申请接口，无 workspace 前缀与认证。

| 接口编号 | 方法 | 路径 | 说明 | 归属模块 |
|----------|------|------|------|----------|
| API-01 | POST | `/documents` | 上传文档 | Upload |
| API-02 | GET | `/documents/{documentId}` | 获取文档状态 | Upload |
| API-03 | GET | `/documents/{documentId}/pages` | 获取页面列表 | Viewer |
| API-04 | POST | `/documents/{documentId}/pages/signed-url` | 获取签名 URL | Viewer |
| API-05 | POST | `/events` | 上报阅读事件 | Analytics |
| API-06 | POST | `/search` | 文档内搜索 | Search |
| API-07 | POST | `/assistant/chat` | AI 问答 | Assistant |
| API-08 | POST | `/links` | 创建链接 | Link |
| API-09 | GET | `/?tenant={tenantSlug}&workspace={workspaceSlug}&token={linkToken}` | 访问公开链接（自定义域名根路径） | Link |
| API-10 | GET | `/analytics/links/{linkId}/score` | 热度评分 | Analytics |
| API-11 | GET | `/analytics/links/{linkId}/suggestions` | 跟进建议 | Analytics |
| API-12 | POST | `/deal-rooms` | 创建数据室 | Deal Room |
| API-13 | GET | `/deal-rooms/{roomId}` | 获取数据室 | Deal Room |
| API-14 | POST | `/public/deal-rooms/{roomId}/access-requests` | 访问申请/审批 | Deal Room |
| API-15 | POST | `/integrations/crm/sync` | CRM 同步 | Integration |
| API-16 | POST | `/integrations/slack/notify` | Slack 通知 | Integration |
...
| 链接权限设置 | 待补充（预计 2026-07-09） | 待设计 | 安全强度 vs 摩擦提示 |
| Viewer 阅读页 | 待补充（预计 2026-07-09） | 待设计 | Canvas + 悬浮 AI + 目录 |
| AI 问答对话框 | 待补充（预计 2026-07-09） | 待设计 | 答案 + 引用 + 页码跳转 |
| 数据分析页 | 待补充（预计 2026-07-09） | 待设计 | 热度、页面停留、访问者 |
| 数据室页 | 待补充（预计 2026-07-09） | 待设计 | 文件夹、权限、Q&A |
| 设置页 | 待补充（预计 2026-07-09） | 待设计 | 品牌、集成、成员 |

> **说明**：设计稿当前尚未产出，计划在 2026-07-09 里程碑前完成。PRD 评审通过前需确认设计排期。
...
> **说明**：以上清单已于 2026-06-20 PRD 评审会上逐项确认、勾选并签字；PRD-v2.1.0 已批准进入 TDD 阶段。
...
> 详细数据流图（含文档上传、文档查看、AI 问答）已迁移至 `docs/ARCHITECTURE-v2.1.0.md` 第 4 章。
>
> 参见：[ARCHITECTURE-v2.1.0.md#4-数据流图](ARCHITECTURE-v2.1.0.md)
...
| 里程碑 | 日期 | 交付物 | 通过标准 |
|--------|------|--------|----------|
| PRD 评审通过 | 2026-06-25 | 已批准 PRD-v2.1.0 | 所有关键方签字 |
| 技术方案确认 | 2026-07-02 | TDD-v2.1.0 + ARCHITECTURE-v2.1.0 | 架构评审通过 |
...
| D-25 | 冻结 PRD-v2.0.7，按 PRD-template-v2 重构为 PRD-v2.1.0 | 在 v2.0.7 上直接 patch | 补齐 YAML front matter、统一版本与 API 路径、清理占位符、明确待确认事项 | 历史版本归档，变更记录需同步 | 产品 |
...
- `docs/PRD-v2.0.7-ARCHIVED.md`（已冻结）
- `docs/PRD + 产品设计的完整文档草案.md`（已冻结）
- `docs/上传-查看-AI问答设计文.md`（已冻结）
- `docs/TDD-v2.1.0.md`（待编写）
- `docs/ARCHITECTURE-v2.1.0.md`（待编写）
- `docs/database-model-v2.1.0.md`（待编写）
- `docs/IMPLEMENTATION-PLAN-v2.1.0.md`（待编写）
- `docs/QA-TEST-PLAN-v2.1.0.md`（待编写）

### 19.3 PRD 评审检查清单

- [x] 产品战略与成功标准被所有关键方理解
- [x] 范围边界清晰，In/Out of Scope 无歧义
- [x] 三类核心用户与关键路径完整
- [x] 16 个 FR 都已详细定义并关联 AC/EVT
- [x] 数据模型与接口契约已确认
- [x] 32 条验收标准覆盖正常/异常/边界/权限/性能/安全/租户隔离/注册流程
- [x] 非功能需求可量化、可测试
- [x] 风险与回滚方案已准备
- [x] 合规、安全、隐私、AI 可信要求已纳入
- [x] 实施计划与里程碑合理可行
...
> 本文档为 DealSignal 的生产级 PRD v2.1.0，基于 `docs/PRD-v2.0.7-ARCHIVED.md`、`docs/PRD + 产品设计的完整文档草案.md` 与 `docs/上传-查看-AI问答设计文.md` 整合编制。
> 以下文件自本文档发布之日起冻结为参考材料，不再作为开发实施依据：
> - `docs/PRD-v2.0.7-ARCHIVED.md`
> - `docs/PRD + 产品设计的完整文档草案.md`
> - `docs/上传-查看-AI问答设计文.md`
> 后续所有需求变更、任务拆分、验收测试、上线运维均以本文档 `docs/PRD-v2.1.0.md` 为准。


### 19.6 待确认问题清单

> 以下问题已在 2026-06-20 PRD 评审会上接受当前假设并确认；状态转为“已批准”后进入执行跟踪。

| 问题 | 当前假设 | 确认人 | 截止日期 | 状态 |
|------|----------|--------|----------|------|
| 异步任务队列最终选型 | MVP 使用 Go channel / 本地队列，日事件量千万级后引入 RabbitMQ/SQS/Kafka | 技术负责人 | 2026-07-02 | 已确认 |
| LLM / Embedding 服务商与模型版本 | OpenAI-compatible API，备选 Azure OpenAI / 自托管 bge + vLLM | 技术负责人 | 2026-07-02 | 已确认 |
| Cloudflare URL Signing 密钥管理 | 后端统一签发，CDN 边缘校验；密钥按季度轮转 | 技术负责人 | 2026-07-02 | 已确认 |
| Viewer 动态水印签名字段 | 水印元信息（邮箱、时间、IP）由后端签名后下发，前端 Canvas 绘制 | 技术负责人 | 2026-07-09 | 已确认 |
| 核心页面设计稿产出 | 计划在 2026-07-09 前完成高保真设计稿与交互原型 | 设计负责人 | 2026-07-09 | 已确认 |
| TDD / ARCHITECTURE / database-model 编写 | PRD 评审通过后启动，2026-07-02 完成技术方案确认 | 技术负责人 | 2026-07-02 | 已确认 |
| 隐私政策与 DPA | 法务确认隐私政策与数据处理协议可在上线前完成 | 合规负责人 | 2026-09-10 | 已确认 |

---

## 20. 模板使用说明

1. **复制资源**：将 `docs/templates/PRD-template-v2.md` 复制为项目级 PRD 并按 `{TYPE}-vX.Y.Z.md` 命名。
2. **按章节填写**：不要跳过背景、范围、用户旅程等战略章节；所有 `{占位符}`、`TODO`、`FIXME` 必须在状态转为“已批准”前清理完毕。
3. **编号唯一**：所有 `FR-XX`、`AC-XX`、`EVT-XX`、`API-XX`、`TASK-XX`、`R-XX`、`D-XX` 等编号在文档内保持唯一。
4. **关联打通**：每个 FR 应关联至少一个 AC 和一个 EVT（如适用）；每个 AC 应能转化为测试用例。
5. **评审驱动**：完成初稿后，召开产品+技术+设计+测试+运营+合规联合评审会。
6. **变更管理**：任何需求变更必须更新变更日志，并重新评估依赖、验收标准与风险。
7. **上线复盘**：上线后回填真实数据到第 3.3 节假设验证表，关闭或调整假设。

> **模板版本**：v2  
> **适用场景**：需要跨产品、设计、开发、测试、运营、合规协作的中大型产品需求  
