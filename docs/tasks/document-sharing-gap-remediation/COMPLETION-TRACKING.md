...
| [TASK-SHARE-SHORT-006.md](./TASK-SHARE-SHORT-006.md) | TASK-SHARE-SHORT-006 | 前端三 Tab 弹窗 | frontend | ~75% | Preset 高亮反馈、domain 下拉、微动画；占位开关（qa/fileRequests/index/screenshot）已禁用并加 Coming soon 提示 |
| [TASK-SHARE-SHORT-007.md](./TASK-SHARE-SHORT-007.md) | TASK-SHARE-SHORT-007 | 邀请邮件、访问通知、请求访问 | fullstack | ~100% | 后端访问请求闭环（公共端点 + 审批 + allow-rule + 邀请邮件）已落地并接入每 IP 每 link 5 次/小时限流；前端请求访问 UI 已补齐；真实后端 E2E 新增访问请求流 spec 通过；PR #84 已创建并关联 `Closes #DS-SHARE-017` |
